...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0) # 0 fully deletes all widget references from memory
...
def first_run(self):
    data = ('John', 'Doe', 'John Doe', '111-111-1111', 'jdoe@email.com') #tuple with indices
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
